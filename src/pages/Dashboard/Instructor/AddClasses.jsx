...
const Addclasses = () => {
    ...
    const onSubmit = (data) => {
        ...
        fetch(`https://api.imgbb.com/1/upload?key=${import.meta.env.VITE_IMGBB_KEY}`, {
            ...
            .then(res => res.json())
            .then(photoUrl => {
                data.classPhoto =  photoUrl?.data?.display_url
                const storedClass = {
                    ...data,
                    status: "pending",
        ...
        <div>
            ...
            >
                ...
                <div className="relative my-5 w-2/4 mx-auto rounded-md m-4 p-1 bg-gradient-to-r from-[#83e0f5d3] via-[#ff11116f] to-[#e4de397e]">
                    <label htmlFor="classPhoto" className="bg-[#83e0f5] px-4 py-1 absolute -top-4 left-3 shadow-lg rounded-md">Upload Class Photo</label>
                    <input
                        type="file"
                        id="classPhoto"
